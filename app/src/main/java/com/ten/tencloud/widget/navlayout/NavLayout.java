package com.ten.tencloud.widget.navlayout;
...
public class NavLayout extends LinearLayout {
    ...
    private static final String TAG = NavLayout.class.getSimpleName();
    private NavLayout instance;
    ...
    private int DEFAULT_SELECTED_COLOR = Color.parseColor("#db4f55");
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.NavLayout, defStyleAttr, 0);
        if (typedArray != null) {
            //菜单数量
            mCount = typedArray.getInt(R.styleable.NavLayout_nav_menuCount, 0);
            mTextColorSelected = typedArray.getColor(R.styleable.NavLayout_nav_selectColor,DEFAULT_SELECTED_COLOR);
            mTextColor = typedArray.getColor(R.styleable.NavLayout_nav_defaultColor,DEFAULT_COLOR);
            ...
            menuItem.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (position == mCurrentPosition) {  //已经是选中状态
                        NavLayout.this.OnItemReClick(position);
                    } else {
                        NavLayout.this.OnItemClick(position);
                    }
                }
            });
     ...
     * @param position
     */
    public NavLayout setSelected(int position) {
     ...
     * @return
     */
    public NavLayout setIconRes(int[] listIconRes) {
     ...
     * @return
     */
    public NavLayout setIconResSelected(int[] listIconResSelected) {
     ...
     * @return
     */
    public NavLayout setIconSize(int iconWidth, int iconHeight) {
     ...
     * @return
     */
    public NavLayout setIconSize(int position, int iconWidth, int iconHeight) {
        this.mIconWidth = iconWidth;
        this.mIconHeight = iconHeight;
        if (mCount != mListIconResSelected.length) {
            Log.e(TAG, "the iconResSelected length is not equals count");
     ...
     * @return
     */
    public NavLayout setTextRes(String[] listText) {
     ...
     * @param textSize
     */
    public NavLayout setTextSize(int textSize) {
     ...
     * @return
     */
    public NavLayout setTextSize(int position, int textSize) {
        if (mCount != mListText.length) {
            Log.e(TAG, "the textRes length is not equals count");
        } else {
            if (position >= 0 && position < mCount) {
     ...
     * @param textColor
     */
    public NavLayout setTextColor(int textColor) {
    ...
    public NavLayout setTextColor(int position, int textColor) {
        if (mCount != mListText.length) {
            Log.e(TAG, "the textRes length is not equals count");
        } else {
            if (position >= 0 && position < mCount) {
     ...
     */
    public NavLayout setTextColorSelected(int textColorSelected) {
    ...
    public NavLayout setTextColorSelected(int position, int textColorSelected) {
        if (mCount != mListText.length) {
            Log.e(TAG, "the textRes length is not equals count");
        } else {
            if (position >= 0 && position < mCount) {
     ...
     * @param marginTop
     */
    public NavLayout setMarginTop(int marginTop) {
    ...
    public NavLayout setMarginTop(int position, int marginTop) {
        if (mCount != mListText.length) {
            Log.e(TAG, "the textRes length is not equals count");
        } else {
            if (position >= 0 && position < mCount) {
     ...
     * @param backColor
     */
    public NavLayout setBackColor(int backColor) {
        setBackgroundColor(backColor);
        return instance;
    }

    public NavLayout setBackColor(int position, int backColor) {
        if (position >= 0 && position < mCount) {
     ...
     * @return
     */
    public NavLayout setMsg(int position, String msg) {
        if (position >= 0 && position < mCount) {
     ...
     * @return
     */
    public NavLayout hideMsg(int position) {
        if (position >= 0 && position < mCount) {
     ...
     * @return
     */
    public NavLayout showRedPoint(int position) {
        if (position >= 0 && position < mCount) {
     ...
     * @return
     */
    public NavLayout hideRedPoint(int position) {
        if (position >= 0 && position < mCount) {
